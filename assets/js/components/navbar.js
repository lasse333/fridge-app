...
export default function Navbar() {
    ...
    const nav = createElement("nav", {style: {height: "60px", width: "100%", backgroundColor: "#222", display: "flex"}})

    let tabs = []

    const makeButton = (func, pageUrl, pageUrlPart, buttonText, img) => {
        let newElement = createElement("button", {
            onclick: func, style: buttonStyle
        }, [
            createElement("img", {
    ...
    window.onclick = function() {
        ...
        tabs.map((button, i) => {
            ...
            button.element.querySelector("span").innerText = langText[getLang()]["navbar"][button.lang]
            if (button.element.classList.contains("active")) button.element.classList.remove("active")
    ...
    document.body.appendChild(addChildren(nav, [
